<template>
  ...
  <div class="upload-container">
    <el-upload
        ...
        :file-list="fileList"
        multiple>
      <el-button slot="trigger" size="small" type="primary" class="large-button">Choose files</el-button>
    </el-upload>
  </div>
...
export default {
  ...
  methods: {
    customRequest({file, onProgress, onSuccess, onError}) {
      ...
      axios.post('http://localhost:5050/api/predict', formData, {
        headers: {
    ...
    handleRemove(file, fileList) {
      ...
      this.accents = this.accents.filter(accent => accent.file.name !== file.name);
