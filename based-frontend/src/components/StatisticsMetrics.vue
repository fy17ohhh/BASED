<template>
  <div class="statistics-metrics-container">
    <div v-if="groupedData" v-for="(items, categoryDisplay) in groupedData" :key="categoryDisplay" class="metric-item">
      <h3 class="category-title">{{ categoryDisplay }}</h3>

      <div v-for="item in items" :key="item.id" class="metric-details">
        <div class="metric-row">
          <div class="metric-value-left"
               :class="item.metricName.toLowerCase().replace(/_/g, '-').concat('-value')">
            {{ item.metricValue }}
          </div>
          ...
          <div class="metric-bar-container">
            <div
                ...
                :style="{ width: calculatePercentage(item.metricName, item.metricValue) + '%' }"
            ></div>
            <div class="metric-name">
              {{ item.metricNameDisplay }}
              <el-tooltip v-if="metricDetails[item.metricName]" :content="`${metricDetails[item.metricName]?.metricDescription}`" placement="bottom" @click.stop.prevent>
          ...
          <div class="metric-percentage">
            ...
            <el-tooltip v-if="metricDetails[item.metricName]" :content="`${metricDetails[item.metricName]?.metricRelationToBias}`" placement="bottom" @click.stop.prevent>
              <i class="el-icon-question" />
            </el-tooltip>
          </div>
        </div>
...
const processStatisticsMetricsData = async () => {
  if (props.statisticsMetrics) {
    ...
    const metricNames = [...new Set(props.statisticsMetrics.map(item => item.metricName))];
    for (const metricName of metricNames) {
      try {
        const response = await axios.get(`http://localhost:8081/api/dataset/metric/details?metricName=${metricName}`);
